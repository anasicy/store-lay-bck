...
# ── Wall-fixture types (placed against walls, need 900 mm clearance from floor) ─
WALL_FIXTURE_TYPES = {
...
class GridLayoutEngine:
    ...
    def generate_classic_grid(self) -> Tuple[List[Dict], float]:
        ...
        # 4. Wall displays — left + right walls (RETAIL zone)
        wall = (g.get('IB_FRAME', []) + g.get('LUX_UNIT', []) +
                g.get('HB_YA', []) + g.get('HB_MW', []) +
                g.get('LENS_UNIT', []) + g.get('WALL_DISPLAY', []) +
                g.get('LUXURY', []) + g.get('AFFORDABLE', []))
        half = len(wall) // 2
        self._place_vert(wall[:half], zones['LEFT_WALL'], pl, side='left',
                         zone_override='RETAIL_MID')
        self._place_vert(wall[half:], zones['RIGHT_WALL'], pl, side='right',
                         zone_override='RETAIL_MID')

        # 5. Sunglass wall units
        sun_wall = g.get('SG_WALL', []) + g.get('SG_ANGULAR', [])
        sh = len(sun_wall) // 2
        self._place_vert(sun_wall[:sh], zones['LEFT_WALL'], pl, side='left',
                         zone_override='SUNGLASSES')
        self._place_vert(sun_wall[sh:], zones['RIGHT_WALL'], pl, side='right',
                         zone_override='SUNGLASSES')

        # 6. Kids unit near entrance (left or right wall, front zone)
        kids = g.get('KIDS_UNIT', [])
        if kids:
            self._place_vert(kids, zones['LEFT_WALL'], pl, side='left',
                             zone_override='KIDS')

        # 7. Smart unit on floating pillar / near entrance
        smart = g.get('SMART_UNIT', [])
        if smart:
            self._place_horiz(smart[:1], zones['RETAIL_FRONT'], pl, gap=500,
                              zone_override='SMART')
    ...
    def generate_racetrack_loop(self) -> Tuple[List[Dict], float]:
        ...
        # 4. All wall + affordable units on left/right
        wall_all = (g.get('IB_FRAME', []) + g.get('SG_WALL', []) + g.get('SG_ANGULAR', []) +
                    g.get('LUX_UNIT', []) + g.get('HB_YA', []) + g.get('HB_MW', []) +
                    g.get('LENS_UNIT', []) + g.get('WALL_DISPLAY', []) +
                    g.get('LUXURY', []) + g.get('AFFORDABLE', []))
        half = len(wall_all) // 2
        self._place_vert(wall_all[:half], zones['LEFT_WALL'], pl, side='left',
                         zone_override='RETAIL_MID')
        self._place_vert(wall_all[half:], zones['RIGHT_WALL'], pl, side='right',
                         zone_override='RETAIL_MID')

        # 5. Kids + Smart near entrance
        if g.get('KIDS_UNIT'):
            self._place_vert(g['KIDS_UNIT'], zones['LEFT_WALL'], pl, side='left',
                             zone_override='KIDS')
        if g.get('SMART_UNIT'):
            self._place_horiz(g['SMART_UNIT'][:1], zones['RETAIL_FRONT'], pl, 500,
                              zone_override='SMART')
    ...
    def generate_premium_open(self) -> Tuple[List[Dict], float]:
        ...
        # 4. Luxury + hero displays near entrance (impulse zone)
        hero = (g.get('LUX_UNIT', []) + g.get('FLATBED', [])[:2] +
                g.get('DISPLAY', [])[:1])
        self._place_horiz(hero, zones['RETAIL_FRONT'], pl, 700,
                          zone_override='RETAIL_PREMIUM')

        # 5. Smart unit on floating pillar near entrance
        if g.get('SMART_UNIT'):
            self._place_horiz(g['SMART_UNIT'][:1], zones['RETAIL_FRONT'], pl, 500,
                              zone_override='SMART')

        # 6. Premium wall displays
        ...
        half = len(pw) // 2
        self._place_vert(pw[:half], zones['LEFT_WALL'], pl, side='left', gap=500,
                         zone_override='RETAIL_PREMIUM')
        self._place_vert(pw[half:], zones['RIGHT_WALL'], pl, side='right', gap=500,
                         zone_override='RETAIL_PREMIUM')

        # 7. Sunglass wall units
        sw = g.get('SG_WALL', []) + g.get('SG_ANGULAR', [])
        sh = len(sw) // 2
        self._place_vert(sw[:sh], zones['LEFT_WALL'], pl, side='left', gap=500,
                         zone_override='SUNGLASSES')
        self._place_vert(sw[sh:], zones['RIGHT_WALL'], pl, side='right', gap=500,
                         zone_override='SUNGLASSES')

        # 8. HB-M&W on remaining wall space
        mw = g.get('HB_MW', []) + g.get('AFFORDABLE', [])
        mw_h = len(mw) // 2
        self._place_vert(mw[:mw_h], zones['LEFT_WALL'], pl, side='left', gap=500,
                         zone_override='RETAIL_MID')
        self._place_vert(mw[mw_h:], zones['RIGHT_WALL'], pl, side='right', gap=500,
                         zone_override='RETAIL_MID')

        # 9. Kids unit
        if g.get('KIDS_UNIT'):
            self._place_vert(g['KIDS_UNIT'], zones['LEFT_WALL'], pl, side='left', gap=500,
                             zone_override='KIDS')
